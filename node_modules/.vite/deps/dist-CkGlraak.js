import { n as __exportAll, t as __commonJSMin } from "./rolldown-runtime-B-lAHAz2.js";
import { t as nanoid } from "./index.browser-CHrKi8Kg.js";
import { n as __name } from "./chunk-Y2CYZVJY-CrOJDJJy.js";
import { B as setLogLevel, R as select_default, z as log } from "./src-Bw6MGI6j.js";
import { $ as purify, C as getDiagramLoader, E as getSiteConfig, I as reset, L as sanitizeCss, N as registerDiagram, P as registerLazyLoadedDiagrams, Q as updateSiteConfig, S as getDiagram, T as getEffectiveHtmlLabels, V as saveConfigFromInitialize, W as setConfig, X as styles_default, Z as themes_default, _ as frontMatterRegex, b as getConfig, c as configureSvgSize, g as evaluate, l as cssStyleSheetToString, m as detectors, n as addDirective, p as detectType, q as setSiteConfig, r as assignWithDepth_default, s as common_default, t as UnknownDiagramError, u as defaultConfig, x as getConfig2 } from "./chunk-DU6HZSFF-tuB5OlwI.js";
import { S as isBuffer, a as decodeEntities$1, d as interpolateToCurve, f as isDetailedError, g as utils_default, h as removeDirectives, i as cleanAndMerge, o as encodeEntities$1, v as isTypedArray, x as isArrayLike, y as isArguments } from "./chunk-75Z2AOVW-Cq13IkmX.js";
import "./chunk-P2QGCYS3-dfbrUmbw.js";
import { r as registerIconPacks } from "./chunk-PWAF6VOD-DKGXq5PA.js";
import { i as dedent } from "./chunk-GMAD6QVW-CchTAkE6.js";
import { a as labelHelper } from "./chunk-4HAMMTFA-DJj9RmRe.js";
import { i as positionNode, n as getSubGraphTitleMargins, r as insertNode, t as clear } from "./chunk-GVQU2GXP-oDE4JFlB.js";
import { n as insertCluster, t as clear$1 } from "./chunk-L3NEJ4N5-BGPDg71x.js";
import { a as insertEdgeLabel, c as positionEdgeLabel, i as insertEdge, l as terminalLabels, n as edgeLabels, r as hasEdgeLabel, s as markers_default, t as clear$2 } from "./chunk-OSK3NFVY-CzRbKAgw.js";
	...
	Object.keys(style).forEach((property) => {
		switch (property) {
			...
			case CONTAINER_STYLE_PROPERTY.STROKE_WIDTH:
				...
				break;
			case CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY: excalidrawProperty.strokeStyle = "dashed";
		}
	});
...
var computeExcalidrawVertexLabelStyle = (style) => {
	...
	Object.keys(style).forEach((property) => {
		switch (property) {
			case LABEL_STYLE_PROPERTY.COLOR: excalidrawProperty.strokeColor = style[property];
		}
	});
	return excalidrawProperty;
};
...
var SUBGRAPH_LABEL_WIDTH_RATIO = .62;
...
var computeVertexLabelFontSize = (vertexType, text, width, fontSize) => {
	const safeFontSize = fontSize || 20;
	...
	if (estimateLabelWidth(text, safeFontSize) <= availableWidth) return safeFontSize;
...
var FlowchartToExcalidrawSkeletonConverter = new GraphConverter({ converter: (graph, options) => {
	const elements = [];
	...
	graph.subGraphs.reverse().forEach((subGraph) => {
		...
		const minSubGraphWidth = estimateLabelWidth(subGraphText, fontSize || 16) + 64;
	...
	Object.values(graph.vertices).forEach((vertex) => {
		...
		switch (vertex.type) {
			...
			case VERTEX_TYPE.CIRCLE:
				...
				break;
			case VERTEX_TYPE.DIAMOND: containerElement.type = "diamond";
		}
		elements.push(containerElement);
	});
	graph.edges.forEach((edge) => {
		...
		const points = reflectionPoints.map((point) => localPoint(point.x - reflectionPoints[0].x, point.y - reflectionPoints[0].y));
...
var GraphImageConverter = new GraphConverter({ converter: (graph) => {
...
var transformToExcalidrawLineSkeleton = (line) => {
	const lineElement = {
		...
		x: line.startX,
		y: line.startY,
		points: [point$1(0, 0), point$1(line.endX - line.startX, line.endY - line.startY)],
...
var transformToExcalidrawArrowSkeleton = (arrow) => {
	const arrowElement = {
		...
		x: arrow.startX,
		y: arrow.startY,
		points: arrow.points?.map(([x, y]) => point$1(x, y)) || [point$1(0, 0), point$1(arrow.endX - arrow.startX, arrow.endY - arrow.startY)],
...
var estimateTextWidth = (text, fontSize) => {
	return Math.max(20, Math.round(text.length * fontSize * .6));
};
var addGroupId = (element, groupId, includeLabel = true) => {
	...
	if (!existingGroupIds.includes(groupId)) mutableElement.groupIds = [...existingGroupIds, groupId];
	if (!includeLabel || !mutableElement.label) return;
	const labelGroupIds = mutableElement.label.groupIds ?? [];
	if (!labelGroupIds.includes(groupId)) mutableElement.label.groupIds = [...labelGroupIds, groupId];
};
var SequenceToExcalidrawSkeletonConvertor = new GraphConverter({ converter: (chart) => {
	const elements = [];
	const activations = [];
	...
	if (chart.groups) chart.groups.forEach((group) => {
		...
		actors.forEach((actor) => {
			if (actor.x === void 0 || actor.y === void 0 || actor.width === void 0 || actor.height === void 0) return;
		...
		const groupRectWidth = maxX - minX + 20;
		const groupRectHeight = maxY - minY + 20;
		...
		elements.forEach((ele) => {
			if (ele.id === groupRectId) return;
			if (ele.x === void 0 || ele.y === void 0 || ele.width === void 0 || ele.height === void 0) return;
			if (ele.x >= minX && ele.x + ele.width <= maxX && ele.y >= minY && ele.y + ele.height <= maxY) addGroupId(ele, groupId);
		});
		if (name) {
...
var classToExcalidrawSkeletonConvertor = new GraphConverter({ converter: (chart) => {
	const elements = [];
...
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/converter/types/er.js
var erToExcalidrawSkeletonConvertor = new GraphConverter({ converter: (chart) => {
	const elements = [];
	chart.nodes.forEach((node) => {
		if (!node || !node.length) return;
		node.forEach((element) => {
...
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/converter/types/state.js
var point = (x, y) => [x, y];
...
var STATE_NODE_LABEL_HORIZONTAL_PADDING = 10;
var STATE_NODE_LABEL_VERTICAL_PADDING = 10;
...
var getStateEndGroupIds = (node) => node.shape === "stateEnd" ? [`state_end_group_${node.id}`] : void 0;
var getNodeText = (node) => {
	if (node.shape === "rectWithTitle" && node.description.length) return [node.text, ...node.description].join("\n");
	return node.text;
};
var labelMeasureContext;
var getLabelMeasureContext = () => {
	if (labelMeasureContext !== void 0) return labelMeasureContext;
...
var measureLabelLineWidthAtFontSize = (line, fontSize) => {
	const context = getLabelMeasureContext();
	if (!context) return line.length * fontSize * .6;
	context.font = `${fontSize}px Excalifont, sans-serif`;
	return context.measureText(line).width;
};
var wrapLabelWord = (word, fontSize, maxWidth) => {
	if (measureLabelLineWidthAtFontSize(word, fontSize) <= maxWidth) return [word];
	const wrappedParts = [];
	let currentPart = "";
	...
	if (currentPart) wrappedParts.push(currentPart);
	return wrappedParts;
};
var wrapLabelLine = (line, fontSize, maxWidth) => {
	if (!line.trim()) return line;
	if (measureLabelLineWidthAtFontSize(line, fontSize) <= maxWidth) return line;
	...
	for (const word of words) {
		...
		for (const [wordPartIndex, wordPart] of wrappedWordParts.entries()) {
			const candidateLine = currentLine ? `${currentLine}${currentLine ? wordPartIndex === 0 ? " " : "" : ""}${wordPart}` : wordPart;
		...
		if (wrappedWordParts.length > 1) continue;
	}
	wrappedLines.push(currentLine);
	return wrappedLines.join("\n");
};
var getWrappedLabelMetrics = (lines, fontSize, availableWidth) => {
	const wrappedLines = lines.map((line) => wrapLabelLine(line, fontSize, availableWidth)).join("\n").split("\n");
	return {
		width: Math.max(...wrappedLines.map((line) => measureLabelLineWidthAtFontSize(line, fontSize))),
		height: wrappedLines.length * fontSize * EXCALIDRAW_LINE_HEIGHT
	};
};
var findLargestLabelFontSizeThatFitsBounds = (text, availableWidth, availableHeight, fallbackFontSize) => {
	...
	for (let fontSize = DEFAULT_STATE_NODE_LABEL_FONT_SIZE; fontSize >= MIN_STATE_NODE_LABEL_FONT_SIZE; fontSize -= STATE_NODE_LABEL_FONT_SIZE_STEP) {
		...
		if (height <= availableHeight) return fontSize;
	}
	return fallbackFontSize;
};
var getNodeLabelFontSize = (node) => {
	const text = getNodeText(node);
	if (!text || STATE_PSEUDO_STATE_SHAPES_WITHOUT_LABEL.has(node.shape)) return DEFAULT_STATE_NODE_LABEL_FONT_SIZE;
	...
	const lines = text.split("\n");
	if (lines.length > 1) {
		if (Math.max(...lines.map((line) => measureLabelLineWidthAtFontSize(line, DEFAULT_STATE_NODE_LABEL_FONT_SIZE))) <= availableWidth) return DEFAULT_STATE_NODE_LABEL_FONT_SIZE;
	}
...
var createLabel = (node) => {
	if (STATE_PSEUDO_STATE_SHAPES_WITHOUT_LABEL.has(node.shape)) return;
	const text = getNodeText(node);
	if (!text) return;
	return {
		text,
		fontSize: getNodeLabelFontSize(node),
		verticalAlign: node.shape === "rectWithTitle" || node.shape === "roundedWithTitle" ? "top" : "middle",
		...computeExcalidrawVertexLabelStyle(node.labelStyle)
	};
};
var createContainerElement = (node) => {
	const baseStyle = computeExcalidrawVertexStyle(node.containerStyle);
	const label = createLabel(node);
	const elementType = node.shape === "choice" ? "diamond" : node.shape === "stateStart" || node.shape === "stateEnd" ? "ellipse" : "rectangle";
	const hasRoundedCorners = node.shape === "rect" || node.shape === "rectWithTitle" || node.shape === "roundedWithTitle";
	const isFilledStateNode = node.shape === "stateStart" || node.shape === "fork" || node.shape === "join";
	...
	return {
		id: node.id,
		type: elementType,
		...getStateEndGroupIds(node) ? { groupIds: getStateEndGroupIds(node) } : {},
		x: node.x,
		y: node.y,
		width: node.width,
		height: node.height,
...
var createDividerLineElement = (node) => {
	if (!node.dividerLine) return null;
	const baseStyle = computeExcalidrawVertexStyle(node.containerStyle);
	return {
		...
		points: [point(0, 0), point(node.dividerLine.endX - node.dividerLine.startX, node.dividerLine.endY - node.dividerLine.startY)],
		strokeColor: baseStyle.strokeColor || "#000",
		strokeWidth: baseStyle.strokeWidth || 1
	};
};
var createEndStateInnerEllipse = (node) => {
	const outerStyle = computeExcalidrawVertexStyle(node.containerStyle);
	const inset = Math.max(2, Math.min(node.width, node.height) * .32);
	const fillColor = node.endInnerColor || outerStyle.strokeColor || outerStyle.backgroundColor || DEFAULT_STATE_FILLED_COLOR;
	return {
		id: `${node.id}__inner`,
		type: "ellipse",
...
var stateToExcalidrawSkeletonConvertor = new GraphConverter({ converter: (chart) => {
	const elements = [];
	chart.nodes.forEach((node) => {
		...
		if (dividerLineElement) elements.push(dividerLineElement);
		if (node.shape === "stateEnd") elements.push(createEndStateInnerEllipse(node));
	});
...
var DEFAULT_POINT_DEDUPE_THRESHOLD = .5;
var dedupeConsecutivePoints = (points, threshold = DEFAULT_POINT_DEDUPE_THRESHOLD) => {
	...
	points.forEach((point) => {
		...
		if (Math.hypot(point[0] - previousPoint[0], point[1] - previousPoint[1]) <= threshold) return;
		dedupedPoints.push(point);
	});
...
var getDecodedEdgePoints = (edgePath) => {
	...
	try {
		...
		return Array.isArray(points) ? points.filter((point) => point && typeof point.x === "number" && typeof point.y === "number" && Number.isFinite(point.x) && Number.isFinite(point.y)) : [];
	} catch {
		return [];
	}
};
var computeEdgePositions = (pathElement, offset = {
	x: 0,
	y: 0
}, commandsPattern = "LM") => {
	...
	const reflectionPoints = commands.map((command) => {
		...
		if (index === array.length - 2 && point.command === "C") return false;
...
var normalizeLinearElementPoints = (result) => {
	return {
		...
		elements: result.elements.map((element) => {
			if (!("points" in element) || !Array.isArray(element.points)) return element;
			const points = element.points;
			if (points.length < 2) return element;
			const dedupedPoints = dedupeConsecutivePoints(points);
			if (dedupedPoints.length === points.length) return element;
			return {
				...element,
...
var graphToExcalidraw = (graph, options = {}) => {
	return normalizeLinearElementPoints((() => {
		switch (graph.type) {
			...
			case "erd": return erToExcalidrawSkeletonConvertor.convert(graph, options);
			case "state": return stateToExcalidrawSkeletonConvertor.convert(graph, options);
...
(/* @__PURE__ */ __name(() => {
	registerLayoutLoaders([
		{
			name: "dagre",
			loader: /* @__PURE__ */ __name(async () => await import("./dagre-GXQ25YYZ--pPKyyF4.js"), "loader")
		},
		{
			name: "swimlane",
			loader: /* @__PURE__ */ __name(async () => await import("./swimlanes-42K2YHIH-ti30VZqc.js"), "loader")
		},
		...[{
			name: "cose-bilkent",
			loader: /* @__PURE__ */ __name(async () => await import("./cose-bilkent-JH36ORCC-DAXfZPrG.js"), "loader")
		}]
	]);
...
var c4Detector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./c4Diagram-7LVT6UL2-C6mcMyso.js");
		return {
			id,
			diagram: diagram2
		};
...
var flowDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./flowDiagram-HODETNUW-BvL-2-qA.js");
		return {
			id: id2,
			diagram: diagram2
		};
...
var detector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./swimlanesDiagram-VR7AAH4N--D6cZDJH.js");
		return {
			id: id4,
			diagram: diagram2
		};
...
var erDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./erDiagram-RLTQ6QDP-BExjaWeO.js");
		return {
			id: id5,
			diagram: diagram2
		};
...
var gitGraphDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./gitGraphDiagram-WWUBYQGX-D9EqaJC7.js");
		return {
			id: id6,
			diagram: diagram2
		};
...
var ganttDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./ganttDiagram-EL5Y4UJY-i5z5REKD.js");
		return {
			id: id7,
			diagram: diagram2
		};
...
var info = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./infoDiagram-27XIBGKW-Dvvxx0lC.js");
		return {
			id: id8,
			diagram: diagram2
		};
...
var pie = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./pieDiagram-E7YTZNPT-icy2DDvY.js");
		return {
			id: id9,
			diagram: diagram2
		};
...
var quadrantDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./quadrantDiagram-AXDQQJYC-CBm4aESr.js");
		return {
			id: id10,
			diagram: diagram2
		};
...
var xychartDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./xychartDiagram-S5SC5T6Z-6blnZikA.js");
		return {
			id: id11,
			diagram: diagram2
		};
...
var requirementDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./requirementDiagram-BXWQKSXE-DCF83_q6.js");
		return {
			id: id12,
			diagram: diagram2
		};
...
var sequenceDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./sequenceDiagram-WJ2MYXX4-CI9YwAdc.js");
		return {
			id: id13,
			diagram: diagram2
		};
...
var classDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./classDiagram-ZZMXUADV-CXTqxbv3.js");
		return {
			id: id14,
			diagram: diagram2
		};
...
var classDetector_V2_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./classDiagram-v2-VYDZK3BY-CelSk8JD.js");
		return {
			id: id15,
			diagram: diagram2
		};
...
var stateDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./stateDiagram-D77RDMKH-BJ-f4NBp.js");
		return {
			id: id16,
			diagram: diagram2
		};
...
var stateDetector_V2_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./stateDiagram-v2-MP3YSRHH-c5PixYdx.js");
		return {
			id: id17,
			diagram: diagram2
		};
...
var journeyDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./journeyDiagram-3NMN7TZE-CvhBe-oQ.js");
		return {
			id: id18,
			diagram: diagram2
		};
...
var detector_default3 = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./timeline-definition-24CTP7MA-CYMu77ag.js");
		return {
			id: id20,
			diagram: diagram2
		};
...
var detector_default4 = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./mindmap-definition-YA3MSWOX-BzNYWEb0.js");
		return {
			id: id21,
			diagram: diagram2
		};
...
var detector_default5 = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./kanban-definition-UXKFOSKX-cUYxwFq_.js");
		return {
			id: id22,
			diagram: diagram2
		};
...
var sankeyDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./sankeyDiagram-P5KCCOFB-BHxsLx25.js");
		return {
			id: id23,
			diagram: diagram2
		};
...
var packet = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./diagram-Z3DM3KII-BabnCJsN.js");
		return {
			id: id24,
			diagram: diagram2
		};
...
var radar = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./diagram-UQ7AKVKN-f8gXNcqC.js");
		return {
			id: id25,
			diagram: diagram2
		};
...
var blockDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./blockDiagram-I7D4REHJ-IEzNqlpi.js");
		return {
			id: id26,
			diagram: diagram2
		};
...
var detector_default6 = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./diagram-S7CK7UJ4-CuSxVaA1.js");
		return {
			id: id27,
			diagram: diagram2
		};
...
var architectureDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./architectureDiagram-5GKGNRK7-BIEov3_X.js");
		return {
			id: id28,
			diagram: diagram2
		};
...
var detector_default7 = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./diagram-VSXAHHWV-BrqTw429.js");
		return {
			id: id29,
			diagram: diagram2
		};
...
var ishikawa = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./ishikawaDiagram-5VMMS53U-CEa_LRfR.js");
		return {
			id: id30,
			diagram: diagram2
		};
...
var vennDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./vennDiagram-4TSXK5OY-D-HnS-zI.js");
		return {
			id: id31,
			diagram: diagram2
		};
...
var treemap = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./diagram-VX7I27RA-y8IHXgSn.js");
		return {
			id: id32,
			diagram: diagram2
		};
...
var wardleyDetector_default = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./wardleyDiagram-VM6X3IG4-B7IZz3FK.js");
		return {
			id: id33,
			diagram: diagram2
		};
...
var cynefin = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./cynefinDiagram-5FMLGOSQ-DpPccLsr.js");
		return {
			id: id34,
			diagram: diagram2
		};
...
var railroad = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./railroadDiagram-O6MQD6OU-CSMfVhr0.js");
		return {
			id: id35,
			diagram: diagram2
		};
...
var railroadEbnf = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./ebnfDiagram-PWID7BFC-6uiQdoqZ.js");
		return {
			id: id36,
			diagram: diagram2
		};
...
var railroadAbnf = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./abnfDiagram-VCTEODGH-zAk6xGnO.js");
		return {
			id: id37,
			diagram: diagram2
		};
...
var railroadPeg = {
	...
	loader: /* @__PURE__ */ __name(async () => {
		const { diagram: diagram2 } = await import("./pegDiagram-XKGWAZYB-BRvM4MO0.js");
		return {
			id: id38,
			diagram: diagram2
		};
...
*/
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/parser/cssUtils.js
/**
...
var looksLikeDeclarationStart = (input, index) => {
	...
	while (cursor < input.length && /[a-z-]/i.test(input[cursor])) cursor += 1;
	if (cursor === propertyStart) return false;
	while (cursor < input.length && /\s/.test(input[cursor])) cursor += 1;
	return input[cursor] === ":";
};
var parseCSSDeclarations = (input) => {
	...
	while (cursor < input.length) {
		...
		while (cursor < input.length) {
			...
			if (quote) {
				if (currentChar === quote && input[cursor - 1] !== "\\") quote = null;
				cursor += 1;
				continue;
			}
			...
			if (parenthesesDepth === 0) {
				...
				if (/\s/.test(currentChar) && looksLikeDeclarationStart(input, cursor)) break;
			}
			cursor += 1;
		}
		const value = cleanCSSValue(input.substring(valueStart, cursor));
		if (property && value) declarations.push({
			property,
			value
		});
		if (cursor < input.length && (input[cursor] === ";" || input[cursor] === ",")) cursor += 1;
	}
	return declarations;
};
var isValidCSSColor = (value) => {
	...
	if (typeof CSS !== "undefined" && typeof CSS.supports === "function") return CSS.supports("color", cleanedValue);
	if (typeof document !== "undefined") {
...
var resolveElementTextColor = (node, fallbackColor) => {
	const textNode = node.querySelector("text, foreignObject, div, span, p") || node;
	...
	if (explicitFallbackColor) return explicitFallbackColor;
};
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/parser/flowchart.js
var applyContainerStyleProperty$1 = (style, key, value) => {
	switch (key) {
		case CONTAINER_STYLE_PROPERTY.FILL:
		case CONTAINER_STYLE_PROPERTY.STROKE:
			...
			break;
		case CONTAINER_STYLE_PROPERTY.STROKE_WIDTH:
		case CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY: style[key] = value;
	}
};
var applyLabelStyleProperty$1 = (style, key, value) => {
	if (key === LABEL_STYLE_PROPERTY.COLOR && isValidCSSColor(value)) style[LABEL_STYLE_PROPERTY.COLOR] = value;
};
var applyStyleTextToStyles$1 = (styleText, containerStyle, labelStyle) => {
	if (!styleText) return;
	parseCSSDeclarations(styleText).forEach(({ property, value }) => {
		applyContainerStyleProperty$1(containerStyle, property, value);
		applyLabelStyleProperty$1(labelStyle, property, value);
	});
};
var applyStyleTextToLabelStyle$1 = (styleText, labelStyle) => {
	if (!styleText) return;
	parseCSSDeclarations(styleText).forEach(({ property, value }) => {
...
var applyElementAttributesToContainerStyle$1 = (element, containerStyle) => {
	if (!element) return;
	[
		[CONTAINER_STYLE_PROPERTY.FILL, element.getAttribute("fill")],
		[CONTAINER_STYLE_PROPERTY.STROKE, element.getAttribute("stroke")],
		[CONTAINER_STYLE_PROPERTY.STROKE_WIDTH, element.getAttribute("stroke-width")],
		[CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY, element.getAttribute("stroke-dasharray")]
	].forEach(([key, rawValue]) => {
		const value = cleanCSSValue(rawValue || "");
		if (value) applyContainerStyleProperty$1(containerStyle, key, value);
	});
};
var applyElementAttributesToLabelStyle$1 = (element, labelStyle) => {
	if (!element) return;
	const color = cleanCSSValue(element.getAttribute("fill") || element.getAttribute("color") || "");
	if (isValidCSSColor(color)) labelStyle[LABEL_STYLE_PROPERTY.COLOR] = color;
};
var applyClassStyles = (classId, classes, containerStyle, labelStyle) => {
	if (!(classes instanceof Map)) return;
...
var parseSubGraph = (data, containerEl, classes) => {
	...
	applyStyleTextToStyles$1(el.getAttribute("style"), containerStyle, labelStyle);
	applyStyleTextToStyles$1(shapeEl?.getAttribute("style"), containerStyle, labelStyle);
	applyElementAttributesToContainerStyle$1(shapeEl, containerStyle);
	const labelEl = el.querySelector(".cluster-label text, .cluster-label tspan") || el.querySelector("text");
	applyStyleTextToLabelStyle$1(labelEl?.getAttribute("style"), labelStyle);
	applyElementAttributesToLabelStyle$1(labelEl, labelStyle);
...
var parseVertex = (vertex, containerEl, classes) => {
	...
	if (!node) return;
	let link;
	if (node.parentElement?.tagName.toLowerCase() === "a") link = node.parentElement.getAttribute("xlink:href");
	...
	const containerStyle = {};
	const labelStyle = {};
	if (vertex.classes && classes instanceof Map) (Array.isArray(vertex.classes) ? vertex.classes : [vertex.classes]).forEach((classId) => {
		applyClassStyles(classId, classes, containerStyle, labelStyle);
	});
	vertex.styles?.forEach((styleText) => {
		applyStyleTextToStyles$1(styleText, containerStyle, labelStyle);
	});
	const shapeEl = node.querySelector(".label-container");
	applyStyleTextToStyles$1(shapeEl?.getAttribute("style"), containerStyle, labelStyle);
	applyElementAttributesToContainerStyle$1(shapeEl, containerStyle);
	Array.from(node.querySelectorAll(".label, .nodeLabel, .label text, .label tspan, .label span, .label div")).forEach((element) => {
		applyStyleTextToLabelStyle$1(element.getAttribute("style"), labelStyle);
		applyElementAttributesToLabelStyle$1(element, labelStyle);
	});
	return {
		id: vertex.id,
...
var parseEdge = (edge, edgeIndex, containerEl) => {
	...
	if (!node) throw new Error("Edge element not found");
	const edgePositionData = computeEdgePositions(node, computeElementPosition(node, containerEl));
	edge.length = void 0;
	return {
		...edge,
		...edgePositionData,
...
var parseMermaidFlowChartDiagram = (db, containerEl) => {
	...
	if (verticesData instanceof Map) verticesData.forEach((vertex, id) => {
		vertices[id] = parseVertex(vertex, containerEl, normalizedClasses);
	});
	else if (typeof verticesData === "object" && verticesData !== null) Object.entries(verticesData).forEach(([id, vertex]) => {
		vertices[id] = parseVertex(vertex, containerEl, normalizedClasses);
	});
	const edgeCountMap = /* @__PURE__ */ new Map();
	const edges = (Array.isArray(edgesData) ? edgesData : []).map((edge) => {
		if (!containerEl.querySelector(`[id*="${edge.id}"]`)) return null;
	...
	return {
		type: "flowchart",
		subGraphs: (Array.isArray(subGraphsData) ? subGraphsData : []).map((subgraph) => {
...
var createContainerSkeletonFromSVG = (node, type, opts = {}) => {
	...
	switch (subtype) {
		case "highlight":
			...
			if (bgColor) container.bgColor = cleanCSSValue(bgColor);
			break;
		case "note": container.strokeStyle = "dashed";
	}
	return container;
};
...
var applyRectStyles = (container, rect) => {
	...
	if (fill && fill !== "none") container.bgColor = cleanCSSValue(fill);
	if (stroke && stroke !== "none") container.strokeColor = cleanCSSValue(stroke);
	if (strokeWidth) container.strokeWidth = Number(strokeWidth);
	if (dashArray && dashArray.trim()) container.strokeStyle = "dashed";
};
var parseActor = (actors, containerEl) => {
	...
	const resolveActorLineNode = (actor, topRootNode) => {
		...
		if (actorLineNode) return actorLineNode;
		...
		if (candidateNode.tagName === "line") return candidateNode;
	...
	actorList.forEach((actor) => {
		...
		if (actor.type === "participant") {
			...
			if (topEllipseNode?.id && bottomEllipseActorNode?.id) actorMap[actor.name] = {
...
var computeArrows = (messages, containerEl, actorMap) => {
	...
	arrowNodes.forEach((arrowNode, index) => {
		...
		const arrow = createArrowSkeletonFromSVG(arrowNode, {
			...
			strokeStyle: getStrokeStyle$2(message.type),
		...
		if (from?.topId && to?.topId) {
			arrow.start = {
				type: from.bindType || "rectangle",
				id: from.topId
			};
			arrow.end = {
				type: to.bindType || "rectangle",
				id: to.topId
			};
		}
...
var computeNotes = (messages, containerEl) => {
	...
	noteNodes.forEach((node, index) => {
		...
		const strokeWidth = rect.getAttribute("stroke-width");
		const dashArray = rect.getAttribute("stroke-dasharray");
		if (fill && fill !== "none") note.bgColor = cleanCSSValue(fill);
		if (stroke && stroke !== "none") note.strokeColor = cleanCSSValue(stroke);
		if (strokeWidth) note.strokeWidth = Number(strokeWidth);
		if (dashArray && dashArray.trim()) note.strokeStyle = "dashed";
		notes.push(note);
	});
	return notes;
};
var parseActivations = (containerEl) => {
	...
	activationNodes.forEach((node) => {
		...
		const applyRectStyles = () => {
			...
			if (fill && fill !== "none") rect.bgColor = cleanCSSValue(fill);
			if (stroke && stroke !== "none") rect.strokeColor = cleanCSSValue(stroke);
			if (strokeWidth) rect.strokeWidth = Number(strokeWidth);
			if (dashArray && dashArray.trim()) rect.strokeStyle = "dashed";
		};
		applyRectStyles();
...
var parseMermaidSequenceDiagram = (diagram, containerEl) => {
	...
	const nodes = [];
	const groups = mermaidParser.getBoxes().map((group) => ({
		...group,
		fill: cleanCSSValue(group.fill || "")
	}));
	const bgHightlights = computeHighlights(containerEl);
	const { nodes: actors, lines, actorMap } = parseActor(mermaidParser.getActors(), containerEl);
...
var parseStyleStrings$1 = (styles) => {
	const styleObj = {};
	if (!styles) return styleObj;
	styles.forEach((style) => {
		parseCSSDeclarations(style).forEach(({ property, value }) => {
			if (property && value) styleObj[property] = cleanCSSValue(value);
		});
	});
	return styleObj;
};
...
var getArrowhead = (type) => {
	...
	switch (type) {
		...
		default: arrowhead = "arrow";
	}
	return arrowhead;
};
var accumulateTranslation$2 = (node, stopAt) => {
	let tx = 0;
	let ty = 0;
	let current = node;
...
var simplifyRoutePoints = (points, tolerance = .5) => {
	...
	for (let index = 1; index < points.length - 1; index++) {
		...
		if (!baseLength) continue;
		const distanceFromSegment = Math.abs(baseX * (currentPoint.y - previousPoint.y) - baseY * (currentPoint.x - previousPoint.x)) / baseLength;
		const projection = ((currentPoint.x - previousPoint.x) * baseX + (currentPoint.y - previousPoint.y) * baseY) / (baseLength * baseLength);
		if (distanceFromSegment <= tolerance && projection >= -tolerance && projection <= 1 + tolerance) continue;
...
var getEdgeRoutePoints = (edgePath) => {
	const routePoints = dedupeConsecutivePoints(getDecodedEdgePoints(edgePath).map((point) => [point.x, point.y])).map(([x, y]) => ({
		x,
		y
	}));
...
var movePointAwayFromAdjacent = (point, adjacentPoint, distance) => {
...
var setArrowRoutePoints = (arrow, points) => {
	const dedupedPoints = dedupeConsecutivePoints(points.map((point) => [point.x, point.y])).map(([x, y]) => ({
		x,
		y
	}));
	if (dedupedPoints.length < 2) throw new Error("Arrow route must contain at least two points");
	...
	arrow.points = dedupedPoints.map((point) => [point.x - startPoint.x, point.y - startPoint.y]);
};
var adjustArrowPosition = (arrow) => {
	const routePoints = arrow.points?.map(([x, y]) => ({
		x: arrow.startX + x,
		y: arrow.startY + y
	})).filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
	if (!routePoints || routePoints.length < 2) return arrow;
	...
	if (!shouldUpdateStartArrowhead && !shouldUpdateEndArrowhead) return arrow;
	if (shouldUpdateStartArrowhead) updatedPoints[0] = movePointAwayFromAdjacent(updatedPoints[0], updatedPoints[1], MERMAID_ARROW_HEAD_OFFSET);
...
var applyPathStylingToArrow = (edgePath, arrow) => {
	...
	if (isValidCSSColor(strokeColor) && strokeColor !== "none") arrow.strokeColor = strokeColor;
	if (Number.isFinite(strokeWidth) && strokeWidth > 0) arrow.strokeWidth = strokeWidth;
};
...
var createArrowFromRoutePoints = (routePoints, primaryEdgePath, opts) => {
	if (routePoints.length < 2) throw new Error(`Class diagram edge ${primaryEdgePath?.id || "<unknown>"} is missing usable path points`);
	...
	const arrow = createArrowSkeletion(startPoint.x, startPoint.y, endPoint.x, endPoint.y, {
		...
		...opts,
		points: routePoints.map((point) => [point.x - startPoint.x, point.y - startPoint.y])
	});
	if (primaryEdgePath) applyPathStylingToArrow(primaryEdgePath, arrow);
...
var getSelfRelationEdgePaths = (classId, containerEl) => {
	...
	].map((pathId) => containerEl.querySelector(`path[id="${pathId}"][data-edge="true"]`)).filter((path) => path !== null);
};
var getArrowRoutePoints = (arrow) => {
	return arrow.points?.map(([x, y]) => ({
		x: arrow.startX + x,
		y: arrow.startY + y
	})).filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y)) || [];
};
...
var getClassTextSection = (textNode, classNode) => {
	...
	while (current && current !== classNode) {
		if (current.classList.contains("annotation-group")) return "header";
		if (current.classList.contains("label-group")) return "header";
		if (current.classList.contains("members-group")) return "members";
		if (current.classList.contains("methods-group")) return "methods";
		current = current.parentElement;
	}
	return "other";
};
var parseClasses = (classes, containerEl, lookUpDomId) => {
	...
	Object.values(classes).forEach((classNode) => {
		...
		const groupId = nanoid();
		const classStyles = parseStyleStrings$1(classNode.styles || classNode.cssStyles);
		let lookedUpId;
		try {
		...
		const findByPrefix = (id) => {
			...
			return Array.from(containerEl.querySelectorAll("[id]")).filter((el) => regex.test(el.id))[0];
		};
		...
		const { tx: containerTx, ty: containerTy } = accumulateTranslation$2(containerSource, containerEl);
		const container = {
			type: "rectangle",
			id: classId,
			groupId,
		...
		if (isMeaningfulColor(resolvedFill)) container.bgColor = resolvedFill;
		else container.bgColor = void 0;
		if (isMeaningfulColor(resolvedStroke)) container.strokeColor = resolvedStroke;
		else container.strokeColor = void 0;
		if (resolvedStrokeWidth) container.strokeWidth = Number(resolvedStrokeWidth);
		else container.strokeWidth = void 0;
		if (resolvedDash && resolvedDash.trim().length > 0) container.strokeStyle = "dashed";
		else container.strokeStyle = void 0;
		nodes.push(container);
		[...Array.from(domNode.querySelectorAll("line")), ...Array.from(domNode.querySelectorAll("g.divider path"))].forEach((lineNode) => {
			const { tx, ty } = accumulateTranslation$2(lineNode, containerEl);
			let startX;
			let startY;
			let endX;
			let endY;
			...
			if (container.strokeColor) line.strokeColor = container.strokeColor;
			else line.strokeColor = void 0;
			if (container.strokeWidth !== void 0) line.strokeWidth = container.strokeWidth;
			else line.strokeWidth = void 0;
			if (container.strokeStyle) line.strokeStyle = container.strokeStyle;
			else line.strokeStyle = void 0;
		...
		textElements.forEach((textNode) => {
			...
			if (!rawText) return;
			const boundingBox = textNode.getBBox();
			const { ty } = accumulateTranslation$2(textNode, containerEl);
			...
			if (isForeignObject && (!Number.isFinite(fontSize) || !fontSize)) {
				...
				if (inner) fontSize = parseFloat(getComputedStyle(inner).fontSize || "");
			}
			if (!Number.isFinite(fontSize) || fontSize <= 0) fontSize = Math.max(12, boundingBox.height * .6);
			fontSize = fontSize * .9;
		...
		if (!container.label) {
			...
			if (fallbackHeaderTextElements.length > 0) container.label = {
		...
		parsedTextElements.filter((element) => {
			if (headerTextElements.length > 0) return element.section !== "header";
...
var parseRelations = (relations, classNodes, containerEl, direction) => {
	...
	relations.forEach((relationNode) => {
		...
		if (!node1) throw new Error(`parseRelations: Cannot find node with id ${id1}`);
		if (!node2) throw new Error(`parseRelations: Cannot find node with id ${id2}`);
		const strokeStyle = getStrokeStyle$1(relation.lineType);
		...
		if (id1 === id2) {
			...
			if (!edgePaths.length) throw new Error(`parseRelations: Cannot find rendered SVG edge for relation ${id1} -> ${id2}`);
			...
			if (!edgePath) throw new Error(`parseRelations: Cannot find rendered SVG edge for relation ${id1} -> ${id2}`);
			relationEdgeIndex += 1;
...
var parseMermaidClassDiagram = (diagram, containerEl) => {
	...
	if (classes && Object.keys(classes).length) {
		const classData = parseClasses(classes, containerEl, typeof db.lookUpDomId === "function" ? db.lookUpDomId.bind(db) : void 0);
		nodes.push(classData.nodes);
		lines.push(...classData.lines);
		text.push(...classData.text);
		classNodes.push(...classData.nodes);
	}
	const { arrows, text: relationTitles } = parseRelations(db.getRelations?.() || [], classNodes, containerEl, direction);
	const { notes: noteContainers, connectors } = parseNotes(db.getNotes?.() || [], containerEl, classNodes);
...
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/parser/er.js
...
var accumulateTranslation$1 = (node, stopAt) => {
	let tx = 0;
	let ty = 0;
	let current = node;
	while (current && current !== stopAt) {
...
var getTextContent = (node) => {
	...
	return entityCodesToText(tspans.length ? tspans.map((span) => span.textContent?.trim()).filter(Boolean).join("\n") : node.textContent?.trim() || "");
};
var getFontSize = (node) => {
	const textNode = node.querySelector("text, foreignObject, div, span, p") || node;
	let fontSize = parseFloat(getComputedStyle(textNode).fontSize || "");
	if (!Number.isFinite(fontSize) || fontSize <= 0) fontSize = Math.max(12, node.getBBox().height * .75);
	return fontSize;
};
var parseTextGroup = (group, containerEl, fallbackColor) => {
	...
	const { tx, ty } = accumulateTranslation$1(group, containerEl);
	return {
...
var getDividerLine$1 = (dividerNode, containerEl, groupId, entityId, fallbackStrokeColor, fallbackStrokeWidth, fallbackStrokeStyle) => {
	const { tx, ty } = accumulateTranslation$1(dividerNode, containerEl);
	let startX = 0;
	let startY = 0;
	let endX = 0;
	let endY = 0;
	...
	if (fallbackStrokeColor && isValidCSSColor(fallbackStrokeColor) && fallbackStrokeColor !== "none") line.strokeColor = fallbackStrokeColor;
	if (fallbackStrokeWidth !== void 0) line.strokeWidth = fallbackStrokeWidth;
	if (fallbackStrokeStyle) line.strokeStyle = fallbackStrokeStyle;
	return line;
};
...
var parseEntity = (entity, containerEl) => {
	...
	if (!domNode) throw new Error(`ER entity ${entity.id} not found in rendered SVG`);
	const groupId = entity.attributes.length ? nanoid() : void 0;
	const bbox = domNode.getBBox();
	const { tx, ty } = accumulateTranslation$1(domNode, containerEl);
	const nodeStyle = parseStyleStrings([...entity.cssStyles || [], ...entity.cssCompiledStyles || []]);
	...
	if (isValidCSSColor(fill) && fill !== "none") container.bgColor = fill;
	if (isValidCSSColor(stroke) && stroke !== "none") container.strokeColor = stroke;
	if (strokeWidth && Number.isFinite(strokeWidth) && strokeWidth > 0) container.strokeWidth = strokeWidth;
	if (dashArray && dashArray !== "none") container.strokeStyle = "dashed";
	return {
		container,
		lines: Array.from(domNode.querySelectorAll(".divider path, path.divider, line.divider")).map((dividerNode) => getDividerLine$1(dividerNode, containerEl, groupId, entity.id, container.strokeColor, container.strokeWidth, container.strokeStyle)).filter((line) => line !== null),
		text: attributeGroups.map((group) => createTextSkeleton(group.x, group.y, group.text, {
			id: nanoid(),
			groupId,
			width: group.width,
			height: group.height,
...
var parseRelationship = (edge, containerEl) => {
	...
	if (!edgePaths.length) throw new Error(`ER relationship ${edge.id} not found in rendered SVG`);
	const points = mergeEdgePoints(edgePaths);
	if (points.length < 2) throw new Error(`ER relationship ${edge.id} is missing usable path points`);
	...
	const arrow = createArrowSkeletion(startPoint.x, startPoint.y, endPoint.x, endPoint.y, {
		...
		points: points.map((point) => [point.x - startPoint.x, point.y - startPoint.y])
	});
	if (isValidCSSColor(strokeColor) && strokeColor !== "none") arrow.strokeColor = strokeColor;
	if (Number.isFinite(strokeWidth) && strokeWidth > 0) arrow.strokeWidth = strokeWidth;
	return arrow;
...
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/parser/state.js
var isMeaningfulCSSColor = (value) => {
	const normalizedValue = cleanCSSValue(value || "");
	if (!normalizedValue) return false;
	if (normalizedValue === "none" || normalizedValue === "transparent" || normalizedValue === "rgba(0, 0, 0, 0)" || normalizedValue === "rgba(0,0,0,0)") return false;
	return isValidCSSColor(normalizedValue);
};
var applyContainerStyleProperty = (style, property, value) => {
	switch (property) {
		case CONTAINER_STYLE_PROPERTY.FILL:
		case CONTAINER_STYLE_PROPERTY.STROKE:
			if (isMeaningfulCSSColor(value)) style[property] = cleanCSSValue(value);
			break;
		case CONTAINER_STYLE_PROPERTY.STROKE_WIDTH:
		case CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY: if (cleanCSSValue(value)) style[property] = cleanCSSValue(value);
	}
};
var applyLabelStyleProperty = (style, property, value) => {
	if (property === LABEL_STYLE_PROPERTY.COLOR && isMeaningfulCSSColor(value)) style[LABEL_STYLE_PROPERTY.COLOR] = cleanCSSValue(value);
};
...
var applyElementAttributesToContainerStyle = (element, containerStyle, explicitProperties) => {
	if (!element) return;
	[
		[CONTAINER_STYLE_PROPERTY.FILL, element.getAttribute("fill")],
		[CONTAINER_STYLE_PROPERTY.STROKE, element.getAttribute("stroke")],
		[CONTAINER_STYLE_PROPERTY.STROKE_WIDTH, element.getAttribute("stroke-width")],
		[CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY, element.getAttribute("stroke-dasharray")]
	].forEach(([property, rawValue]) => {
		if (!explicitProperties.has(property)) return;
		if (containerStyle[property]) return;
		const value = cleanCSSValue(rawValue || "");
		if (!value) return;
		applyContainerStyleProperty(containerStyle, property, value);
	});
};
var applyElementAttributesToLabelStyle = (element, labelStyle, explicitProperties) => {
	if (!element) return;
	const candidates = [element, ...Array.from(element.querySelectorAll("text, foreignObject, div, span, p"))];
	for (const candidate of candidates) {
		if (labelStyle[LABEL_STYLE_PROPERTY.COLOR]) break;
		...
		if ((explicitProperties.has(LABEL_STYLE_PROPERTY.COLOR) || explicitProperties.has(CONTAINER_STYLE_PROPERTY.FILL)) && isMeaningfulCSSColor(color)) labelStyle[LABEL_STYLE_PROPERTY.COLOR] = color;
	}
};
...
var getAbsoluteBounds = (node, containerEl) => {
...
var getDividerLine = (nodeEl, containerEl) => {
...
var getExplicitStyleValue = (element, property) => {
	const styleText = element.getAttribute("style");
	if (!styleText) return;
	const declaration = parseCSSDeclarations(styleText).find((entry) => entry.property === property);
...
var pickShapeElement = (elements, strategy) => {
	const candidates = elements.map((element) => ({
		element,
		area: getShapeArea(element)
	})).filter(({ area }) => Number.isFinite(area) && area > 0);
	if (candidates.length === 0) return null;
	return candidates.sort((left, right) => strategy === "largest" ? right.area - left.area : left.area - right.area)[0].element;
};
var getElementShapeColor = (element, explicitProperties) => {
	if (!element) return;
	if (!explicitProperties.has(CONTAINER_STYLE_PROPERTY.FILL) && !explicitProperties.has(CONTAINER_STYLE_PROPERTY.STROKE)) return;
	...
	if (isMeaningfulCSSColor(fill)) return fill;
	if (isMeaningfulCSSColor(stroke)) return stroke;
};
var getEndStateInnerColor = (nodeEl, explicitProperties) => {
	return getElementShapeColor(pickShapeElement(Array.from(nodeEl.querySelectorAll("circle, ellipse, path")), "smallest"), explicitProperties);
};
var trimSharedTrailingLineIndentation = (lines) => {
	if (lines.length < 2) return lines;
	const trailingLines = lines.slice(1);
	const commonIndent = trailingLines.filter((line) => line.trim().length > 0).reduce((minIndent, line) => {
		const indent = line.match(/^\s*/)?.[0].length ?? 0;
		return Math.min(minIndent, indent);
	}, Number.POSITIVE_INFINITY);
	if (!Number.isFinite(commonIndent) || commonIndent <= 0) return lines.map((line) => line.trimEnd());
	return [lines[0].trimEnd(), ...trailingLines.map((line) => line.replace(new RegExp(`^\\s{0,${commonIndent}}`), "").trimEnd())];
};
var getStateLabelText = (node) => {
	return trimSharedTrailingLineIndentation(Array.isArray(node.label) ? node.label.map((entry) => entityCodesToText(entry)) : entityCodesToText(node.label || "").split("\n")).join("\n");
};
var getStateDescription = (node) => {
	if (!node.description) return [];
	return (Array.isArray(node.description) ? node.description : [node.description]).map((entry) => entityCodesToText(entry)).filter((entry) => entry.length > 0);
};
var createNodeElementResolver = (containerEl) => {
	...
	return (node) => {
		...
		for (const selector of selectors) {
			...
			if (element) return markAndReturn(element);
		}
		switch (node.shape) {
			case "divider": return getNextUnusedCandidate(Array.from(containerEl.querySelectorAll("g.statediagram-cluster-alt")));
			case "stateStart": return getNextUnusedCandidate(Array.from(containerEl.querySelectorAll("g.node.default")).filter((element) => element.querySelector("circle.state-start")));
			case "stateEnd": return getNextUnusedCandidate(Array.from(containerEl.querySelectorAll("g.node.default")).filter((element) => !element.querySelector("circle.state-start")));
			default: return null;
		}
	};
};
var findShapeElement = (nodeEl, shape) => {
	switch (shape) {
		case "roundedWithTitle": return nodeEl.querySelector("rect.outer") || nodeEl.querySelector("rect") || nodeEl;
		case "divider": return nodeEl.querySelector("rect.divider") || nodeEl.querySelector("rect") || nodeEl;
		case "rectWithTitle": return nodeEl.querySelector("rect.outer") || nodeEl.querySelector("rect") || nodeEl;
		case "stateStart": return pickShapeElement(Array.from(nodeEl.querySelectorAll("circle, ellipse, path")), "largest") || nodeEl;
		case "stateEnd": return pickShapeElement(Array.from(nodeEl.querySelectorAll("circle, ellipse, path")), "largest") || nodeEl;
		default: return nodeEl.querySelector("rect, path, circle, ellipse, polygon") || nodeEl;
	}
};
var parseStateNode = (node, containerEl, resolveNodeElement) => {
	const nodeEl = resolveNodeElement(node);
	if (!nodeEl) throw new Error(`State node element not found for "${node.id}"`);
	...
	applyElementAttributesToContainerStyle(shapeEl, containerStyle, explicitStyleProperties);
	applyElementAttributesToLabelStyle(nodeEl, labelStyle, explicitStyleProperties);
	...
	return {
		...
		containerStyle,
		labelStyle,
		dividerLine: node.shape === "rectWithTitle" ? getDividerLine(nodeEl, containerEl) : void 0,
		endInnerColor: node.shape === "stateEnd" ? getEndStateInnerColor(nodeEl, explicitStyleProperties) : void 0,
...
var parseStateEdge = (edge, containerEl) => {
	...
	const edgePositionData = computeEdgePositions(edgeEl, {
		x: tx,
		y: ty
	}, "MCL");
	if (edgePositionData.reflectionPoints.length < 2) return null;
	const edgeStyle = {};
	const applyEdgeStyleProperty = (property, value) => {
		switch (property) {
			case CONTAINER_STYLE_PROPERTY.STROKE:
				if (isMeaningfulCSSColor(value)) edgeStyle.strokeColor = cleanCSSValue(value);
				break;
			case CONTAINER_STYLE_PROPERTY.STROKE_WIDTH: {
				const strokeWidth = parseFloat(cleanCSSValue(value));
				if (Number.isFinite(strokeWidth) && strokeWidth > 0) edgeStyle.strokeWidth = strokeWidth;
				break;
			}
			case CONTAINER_STYLE_PROPERTY.STROKE_DASHARRAY: if (cleanCSSValue(value)) edgeStyle.strokeStyle = "dashed";
		}
	};
...
var parseMermaidStateDiagram = (db, containerEl) => {
	...
	return {
		type: "state",
		nodes: nodes.map((node) => parseStateNode(node, containerEl, resolveNodeElement)),
		edges: edges.map((edge) => parseStateEdge(edge, containerEl)).filter((edge) => edge !== null)
	};
};
//#endregion
//#region node_modules/@excalidraw/mermaid-to-excalidraw/dist/mermaidExecutionQueue.js
...
var runMermaidTaskSequentially = (task) => {
	...
	mermaidTaskQueue = run.then(() => void 0, () => void 0);
	return run;
};
//#endregion
...
var parseMermaid = async (definition, config = MERMAID_CONFIG) => {
	return runMermaidTaskSequentially(async () => {
		...
		const diagram = await mermaid_default.mermaidAPI.getDiagramFromText(encodeEntities(definition));
...
var dist_exports = /* @__PURE__ */ __exportAll({ parseMermaidToExcalidraw: () => parseMermaidToExcalidraw });
...
//#endregion
export { load as a, createCommonLayoutRenderer as c, insertMeasuredNode as d, sortNodesByHierarchy as f, JSON_SCHEMA as i, createLayoutElementGroups as l, getRegisteredLayoutAlgorithm as n, adjustClustersAndEdges as o, selectSvgElement as p, parseMermaidToExcalidraw, render$1 as r, clusterDb as s, dist_exports as t, findNonClusterChild as u };
